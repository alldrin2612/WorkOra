...
import Selectedfreelancer from "./selectedevent_components/Selectedfreelancer";
...
export default function SelectedEvent() {
  ...
  const [confirmedfreelancers, setConfirmedfreelancers] = useState([]); // State to store confirmed freelancers data
  const [unconfirmedfreelancers, setUnconfirmedfreelancers] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
          // Filter freelancers based on confirmation status
          const confirmed = response.data.filter(freelancer => freelancer.confirmed === true);
        const unconfirmed = response.data.filter(freelancer => freelancer.confirmed === false && freelancer.status !="Rejected");
        setConfirmedfreelancers(confirmed); // Update state with confirmed freelancers data
        setUnconfirmedfreelancers(unconfirmed);
        ...
        <Box>
          ...
          >
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <IconButton component={NavLink} to={`/business/event/edit-event/${id}`} aria-label="delete" size="large" sx={{"&:hover":{backgroundColor:'#252526', color:"white"}}}>
          ...
          <Divider/>
            ...
            >
              <Typography sx={{fontSize:18,}}>List Of freelancer</Typography>
              <Box sx={{display:"flex"}}>
          ...
          fullWidth
        >
          <DialogTitle>freelancer Requested</DialogTitle>
          <DialogContent>
          {unconfirmedfreelancers.length > 0 ? (
              <Grid container spacing={2}>
                {unconfirmedfreelancers.map((freelancer) => (
                  <Grid item xs={12} key={id}>
                    <EventCard
                      freelancerName={freelancer.freelancer.name}
                      userName={freelancer.freelancer.user}
                      slug={user.business[0].slug}
                      event={freelancer.event}
                      freelancerId={freelancer.freelancer.id}
                      reloadfreelancers={reloadfreelancers}
              ...
              <Typography variant="body1">No unconfirmed freelancers added.</Typography>
            )}
          </DialogContent>
        </Dialog>
            ...
            {confirmedfreelancers.length > 0 ? (
              <Grid container spacing={2}>
                {confirmedfreelancers.map((freelancer) => (
              ...
              <Typography variant="body1">No confirmed freelancers added.</Typography>
            )}
            </Box>
        </Box>
      )}
