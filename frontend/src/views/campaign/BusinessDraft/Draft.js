...
function Draft() {
  ...
  const { freelancerId, campaignId, amount } = useParams();
...
const contract = new web3.eth.Contract(
  ...
  '0xa37348CA4f597A5d1CfE86a76f4c15fdD35F36d5'// Replace with your contract address
);
  ...
  const handleAccept = async () => {
    ...
    // Call the makeTransactionTofreelancer function
    const transaction = await contract.methods.makeTransactionTofreelancer(campaignData.campaign.name, campaignData.freelancer.name, amountInWei).send({ from: defaultAccount, value : amountInWei,  gas: 100000 });
    ...
    try {
      ...
      const secondResponse = await axios.post(
        `https://influverse-backend.onrender.com//api/interface-buisness/freelancer/work/accept/${campaignData.id}`,
        {
          confirmed : true
        },
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const secondResponse = await axios.patch(
        `https://influverse-backend.onrender.com/api/interface-buisness/freelancer/work/${campaignData.id}`,
        {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://influverse-backend.onrender.com/api/interface-buisness/freelancer/${freelancerId}/campaign-work/${campaignId}/`,
      )
      .then((response) => {
            ...
            >
              ...
              >
                <Typography sx={{ fontSize: 16, fontWeight: "bold" }}>
                  Video Uploaded by {campaignData.freelancer.name}
                </Typography>
              ...
              <Box sx={{ marginY: "10px" }}>
                ...
                {campaignData.comments ? (
                  <CommentsCard
                    ...
                    name={campaignData.freelancer.name}
                    image={campaignData.freelancer.image}
                  />
                ) : (
                  <Typography>No comment by {campaignData.freelancer.name}</Typography>
                )}
                {comment ? (
                  <CommentsCard
                    comment={comment}
