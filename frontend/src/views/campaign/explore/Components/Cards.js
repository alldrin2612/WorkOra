...
export default function Cards({ name, industry, country, id, campaignId,image }) {
  ...
  const handleClick = async () => {
    try {
      const response = await fetch(`https://influverse-backend.onrender.com/api/interface-buisness/${user.business[0].slug}/campaigns/${campaignId}/add-freelancer`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Error adding freelancer:', response.statusText);
      }
    } catch (error) {
    ...
    >
      ...
      >
        <DialogTitle id="alert-dialog-title">
          How Much do you want to pay freelancer ?
        </DialogTitle>
        <DialogContent>
