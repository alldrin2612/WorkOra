  ...
  useEffect(() => {
    const fetchStartupStats = async () => {
      try {
        ...
        const response = await fetch('http://localhost:4000/api/startup/campaign-stats', {
          method: 'POST',
          headers: {
        ...
        <>
          ...
          <Grid item xs={12}>
            <Grid container spacing={gridSpacing}>
              ...
              <EarningCard isLoading={isLoading} totalCampaigns={stats?.totalCampaigns} />
              </Grid>
              <Grid item lg={4} md={6} sm={6} xs={12}>
              <TotalView isLoading={isLoading} completedCampaigns={stats?.completedCampaigns} />
              </Grid>
